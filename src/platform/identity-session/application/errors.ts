...
export function identityError(kind: IdentityErrorKind, cause?: unknown): AppError {
  ...
  const notFound = kind === "SESSION_NOT_FOUND";
  const conflict = kind === "ACCOUNT_USERNAME_CONFLICT" || kind === "BOOTSTRAP_ALREADY_COMPLETED";
  return new AppError({
    ...
    publicMessage: authFailure ? "用户名或密码错误。" : required ? "需要认证。" : limit ? "当前账号已达到有效 Session 数量上限。" : "请求无法完成。",
    cause,
  });
}
