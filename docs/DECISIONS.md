...
| D-023 | 已确认 | V2 数据库基线采用 PostgreSQL 17 与 Prisma | 为真实事务验证建立明确、受支持的数据库基础 | 当前测试数据库实际版本为 PostgreSQL 17.10；集成测试必须验证 PostgreSQL 主版本为 17，并且只使用 `TEST_DATABASE_URL`；V2 尚无业务表、Migration 或历史数据，因此不存在版本升级 Migration 成本；连接信息不得写入已跟踪文件、日志或提交 | PostgreSQL 16 基线、以 Mock 代替真实 PostgreSQL 事务验证 | 2026-08-02 |
