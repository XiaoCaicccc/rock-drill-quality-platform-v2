...
修改产品范围、领域模型、架构或长期决策前，必须停止实施并请求用户明确批准。获批后，先更新 `docs/DECISIONS.md` 及相应权威文档，再修改代码。允许依据许可证正常使用开源框架、库、依赖和必要的官方使用示例；禁止直接复制 Carbon、InvenTree、Cascadia PLM、ERPNext、Part-DB 等第三方制造业业务系统的源码或受许可证约束业务实现；仅允许吸收其设计思想并独立实现本项目业务能力。
